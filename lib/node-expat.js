...
class XmlStream extends Writable {
     ...
     * @param {BufferEncoding} [options.disableBackpressure = false]
     */
    constructor(options = {}) {
        const { highWaterMark = 16384, emitClose = true, autoDestroy = true, defaultEncoding = null, disableBackpressure = false } = options;

        super({
            highWaterMark: disableBackpressure === true ? Number.MAX_VALUE : highWaterMark,
            decodeStrings: true,
            objectMode: false,
            emitClose,
            autoDestroy
    ...
    getError(chunk) {
        ...
        if (err.byteIndex != null && chunk.length) {
